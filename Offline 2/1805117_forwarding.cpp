...
//		}
	...
	bool forward_check(int r, int c, int no)
	{
		set<int>::iterator itr;
		for(int i = 0; i < N; i++)
		{
			if(i != r && domain[i][c].size() == 1)
			{
				itr = domain[i][c].begin();
				if(*itr == no)
				{
					return false;
				}
		...
		for(int j = 0; j < N; j++)
		{
			if(j != c && domain[r][j].size() == 1)
			{
				itr = domain[r][j].begin();
				if(*itr == no)
				{
					return false;
				}
