    ...
    def add_knowledge(self, cell, count):
        ...
        for sentence in self.knowledge:
            ...
            for safe_cell in sentence.known_safes():
                other_safes.add(safe_cell)
            for mine_cell in sentence.known_mines():
                other_mines.add(mine_cell)

        for safe_cell in other_safes:
            self.mark_safe(safe_cell)
        for mine_cell in other_mines:
            self.mark_mine(mine_cell)

        other_sentences = []

        for sentence1, sentence2 in itertools.combinations(self.knowledge, 2):
            infer = sentence1.inference(sentence2)
            if infer is not None and infer not in self.knowledge:
                other_sentences.append(infer)

        for sentence in other_sentences:
            if len(sentence.cells) == 0:
                other_sentences.remove(sentence)
                continue
        self.knowledge = self.knowledge + other_sentences
